...
| [aws/](aws/overview.md) | **Phases 1–6 complete, Phase 7 not started** — [question-taxonomy.md](aws/question-taxonomy.md) (44 concept groups, incl. this track's own Phase-2 gap-hunt) → [roadmap.md](aws/roadmap.md) → 44 planned pages, awaiting page-writing. `aws/roadmap.html` and the root tile's badge are already wired per the Phase-1 mandate. Sits in the ordered core-backend path right after Azure, per the user's own structural instruction. |
